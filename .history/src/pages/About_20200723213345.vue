...
let dbPromise, iDB,
  ...
  initDB = async () => {
    ...
    const iDB = {
      async get(key) {
        return dbPromise.get('keyval', key);
      },
      async set(key, val) {
        return dbPromise.put('keyval', val, key);
      },
      async delete(key) {
        return dbPromise.delete('keyval', key);
      },
      async clear() {
        return dbPromise.clear('keyval');
      },
      async keys() {
        return dbPromise.getAllKeys('keyval');
      },
    };
    return iDB
  }
...
export default {
  methods: {
    ...
    cellUpdated: async (newData) => {
      ...
      const val = (await store.get(newData.id)) || newData;
      console.log('current record:',val);
      await store.put(newData, newData.id);
      await tx.done;
