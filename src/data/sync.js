...
worker.onmessage = async function messageFromWebWorker(event) {
    ...
    if(isOffDBready) {
        ...
        pendingInitPromise.resolve(await addFriendshipsImpure(await offDB.people.toArray()))
    } else {
...
const addFrienshipsLater = async function addFrienshipsLater(referenceToReset,propname='dataset') {
    const everyone = await offDB.people.toArray()
    if(everyone.length) {
        console.log('Friendships should show up in 2sec')
        setTimeout(async ()=>{
            console.log('Trying to set the dataset:',referenceToReset)
            referenceToReset[propname]=await addFriendshipsImpure(everyone)
        },2000)
    }
}


export { getEveryone, addFrienshipsLater, offDB }